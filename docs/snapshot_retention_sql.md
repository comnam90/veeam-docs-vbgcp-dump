...
During every successful backup session, Veeam Backup for Google Cloud creates a new restore point. If Veeam Backup for Google Cloud detects that the number of restore points in the snapshot chain exceeds the retention limit, it removes the earliest restore point from the chain. For more information on the snapshot deletion process, see [Google Cloud documentation](https://cloud.google.com/sql/docs/mysql/backup-recovery/backing-up#delete-backup).

![SQL Snapshot Retention](images/snapshot_retention.webp)
