...
For each date when an update was installed, the Veeam Updater page will display the name of the update and its status (whether the installation process completed successfully, completed with warnings or failed to complete). To download logs for the installed updates, select the necessary date in the Date section and click View Full Log — Veeam Backup for Google Cloud will save the logs as a single file to the default download directory on the local machine.
